...
import { StudentAssignmentsOverview } from "@/components/homework/StudentAssignmentsOverview";
import { getStudentRoomTaskOverview } from "@/lib/student-assignments";
...
export default async function AssignmentsPage({ params }: Props) {
  ...
  if (session.user.role === "TUTOR") {
    redirect(localePath(locale, "/dashboard/materials"));
  }

  const rooms = await getStudentRoomTaskOverview(session.user.id);
  ...
  return (
    <section className="mx-auto max-w-6xl px-4 py-6 sm:px-6 sm:py-8">
      ...
      <StudentAssignmentsOverview rooms={rooms} />
    </section>
  );
}
